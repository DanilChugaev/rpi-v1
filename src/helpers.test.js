...
const { color, position } = require('./helpers');

describe('helpers', () => {
    describe('color', () => {
        test('Должен вернуть битовый формат красного цвета', async () => {
            expect(color({ red: 255 })).toBe(65280);
        });

        test('Должен вернуть битовый формат зеленого цвета', async () => {
            expect(color({ green: 255 })).toBe(16711680);
        });

        test('Должен вернуть битовый формат синего цвета', async () => {
            expect(color({ blue: 255 })).toBe(255);
        });

        test('Должен вернуть битовый формат красного и зеленого цветов', async () => {
            expect(color({ red: 255, green: 255 })).toBe(16776960);
        });

        test('Должен вернуть битовый формат красного и синего цветов', async () => {
            expect(color({ red: 255, blue: 255 })).toBe(65535);
        });

        test('Должен вернуть битовый формат зеленого и синего цветов', async () => {
            expect(color({ green: 255, blue: 255 })).toBe(16711935);
        });

        test('Должен вернуть битовый формат красного, зеленого и синего цветов', async () => {
            expect(color({ red: 255, green: 255, blue: 255 })).toBe(16777215);
        });
    });
    ...
    describe('position', () => {
        ...
        testCases.forEach(({ x, y, expected }) => {
            test(`Должен вернуть местоположение: ${expected}, светодиода по указанным x: ${x} и y: ${y}`, async () => {
                expect(position({ x, y })).toBe(expected);
            });
        });
    });
});
